 ...
 */
public class PeerIdTypeTest {

    @Test
    public void shouldCreateAlphabeticString() {
        for (int i = 0; i < 30; ++i) {
            assertThat(ALPHABETIC.generateString(100))
                    .hasSize(100)
                    .matches(Pattern.compile("\\p{Alpha}+"));
        }
    }
    ...
    public void shouldCreateNumericString() {
        for (int i = 0; i < 30; ++i) {
            assertThat(NUMERIC.generateString(100))
                    .hasSize(100)
                    .matches(Pattern.compile("\\p{Digit}+"));
        }
    }
    ...
    public void shouldCreateAlphanumericString() {
        for (int i = 0; i < 30; ++i) {
            assertThat(ALPHANUMERIC.generateString(100))
                    .hasSize(100)
                    .matches(Pattern.compile("\\p{Alnum}+"));
        }
    }
    ...
    public void shouldCreatePrintableString() {
        for (int i = 0; i < 30; ++i) {
            assertThat(PRINTABLE.generateString(100))
                    .hasSize(100)
                    .matches(Pattern.compile("\\p{Print}+"));
        }
    }
